...
void callback(const sensor_msgs::PointCloud2::ConstPtr& input, const sensor_msgs::NavSatFix::ConstPtr& coordinates){
    ...
    ref.read_file(file_ref,reference_raw);   //Point cloud from CAD file
    ...
    //Get coordinates of the root from the CAD model
    pcl::PointXYZ minPt;
    pcl::PointXYZ maxPt;
    ...
    ref.apply_filter(1,reference_raw,reference_pre);  //Passthrough filter
    ...
    pcl::transformPointCloud (*reference, *reference_centered, center_origin);

    //Filter and segment blade from Lidar measurement
    measure.set_filter(x_min,x_max,y_min,y_max,z_min,z_max,leaf,deviation);
    measure.apply_filter(1,source_cloud, pre_filtered); //Passthrough filter
    measure.apply_filter(2,pre_filtered, filtered_cloud); //Voxel filter
    measure.apply_filter(3,filtered_cloud, filtered_cloud); //Outlier removal filter
    ...
    pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_icp = measure.compare_clouds(3, blade,reference_centered, icp_distance, epsilon);

    // Save final transformation matrix and the icp score of the alignment
    Eigen::Matrix4f final_transform = measure.get_transform();
    final_score = measure.get_score();
