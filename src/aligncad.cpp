...
int main(int argc, char **argv)
{
    ...
    //Declare point cloud variables to store the resulting reference cloud and measured_cloud
    ...
    while (ros::ok())
    {
        ...
        reference_pointcloud.read_file(file_ref); //Store the reference point cloud into a variable
        ...
        reference_pointcloud.apply_filter(Pass_through); //Pass through filter
        reference_pointcloud.apply_filter(Voxel); //Voxel filter

        //Filter blade from measured cloud
        measured_pointcloud.read_file(file); //Store measured point cloud into a variable
        measured_pointcloud.set_filter(measured_cloud_parameters);
        measured_pointcloud.apply_filter(Pass_through); //Pass through filter
        measured_pointcloud.apply_filter(Voxel); //Voxel filter
        measured_pointcloud.apply_filter(Outlier); //Voxel filter
        ...
        filtered_cloud = measured_pointcloud.get_cloud();
        ...
        pcl::transformPointCloud (*filtered_cloud, *transformed_cloud, transform); //Execute transformation
        ...
        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_icp_temp1 = measured_pointcloud.compare_clouds(icp,transformed_cloud, reference_cloud, icp_distance, epsilon);

        pcl::PointCloud<pcl::PointXYZ>::Ptr cloud_icp = measured_pointcloud.compare_clouds(incremental_icp, transformed_cloud,reference_cloud, icp_distance, epsilon);
        Eigen::Matrix4f final_transform = measured_pointcloud.get_transform();
        double final_score = measured_pointcloud.get_score();
